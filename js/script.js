...
async function initBook() {
  try {
    // Primero pedimos el archivo nosotros mismos con fetch(), en vez de dejar
    // que PDF.js pida el PDF directo por URL. Esto evita problemas de rangos
    // (Range requests) que algunos hostings/CDNs no manejan bien, y nos deja
    // revisar el archivo ANTES de tratar de leerlo como PDF (tamaño real,
    // código de estado, si es texto en vez de binario, etc.) para poder
    // mostrar un error específico y útil si algo sale mal.
    const res = await fetch(PDF_URL, { cache: "no-store" });
    if (!res.ok) {
      throw new Error(
        `El servidor respondió ${res.status} (${res.statusText}) al pedir "${PDF_URL}". ` +
        `Revisa que el archivo exista exactamente con ese nombre en la raíz del sitio.`
      );
    }
    const buffer = await res.arrayBuffer();

    // Un PDF real de un catálogo pesa cientos de KB o varios MB. Si llega
    // sospechosamente chico, casi siempre es porque el hosting está sirviendo
    // un "puntero" de Git LFS (un archivo de texto) en vez del PDF real.
    if (buffer.byteLength < 10000) {
      const preview = new TextDecoder().decode(buffer).slice(0, 180);
      throw new Error(
        `"${PDF_URL}" solo pesa ${buffer.byteLength} bytes — no es un PDF real. ` +
        `Esto casi siempre pasa cuando GitHub sirve un puntero de Git LFS en vez ` +
        `del archivo. Contenido recibido: ${JSON.stringify(preview)}`
      );
    }
    ...
    showBookError(err && err.message ? err.message : String(err));
  }
}

function showBookError(detail) {
  bookLoading.hidden = true;
  bookError.hidden = false;
  const isFileProtocol = location.protocol === "file:";
  const friendly = isFileProtocol
  ...
  bookErrorMsg.innerHTML =
    friendly +
    (detail
      ? `<br><br><span class="book-error-detail-label">Detalle técnico:</span>` +
        `<pre class="book-error-detail">${escapeHtml(detail)}</pre>`
      : "");
}
